...
mars_params <- extract_parameter_set_dials(mars_model) %>% 
  update(
    num_terms = num_terms(range = c(50, 80)),
    prod_degree = prod_degree(range = c(6,10))
  )

# build tuning grid 
mars_grid <- grid_regular(mars_params, level = 30)
...
save(tuned_mars_1, file = here("results/round_3/fitted_tuned_models/tuned_mars_1.rda"))